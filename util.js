...
function assert(val, mesg) {
  if (!val) {
    throw new Error(mesg || 'Assertion Failed');
  }
}
...
/**
 * Given a node, return an array containing the node and its
 * parents in bottom-up ord.
 *
 * @param {Object} node The node for which ancestry is retrieved.
 * @param {Function<Node, Node>} [parentOfNode] Function which
 *   takes a node and returns its parent.  If the return value
 *   is null, the iteration will stop.  By default this is a function
 *   which retrieves the 'parent' attribute of the node.
 */
function ancestry(node, parentOfNode) {
  if (!getParent) {
    parentOfNode = n => n.parent;
  }
  const anc = [node];
  let parent = parentOfNode(node);
  while (parent) {
    anc.push(parent);
...
function attr(obj, prop, initializer) {
  assert(typeof initializer === 'function');
  if (!(prop in obj)) {
    obj[prop] = initializer(obj, prop);
  }
  return obj[prop];
}
exports.attr = attr;
